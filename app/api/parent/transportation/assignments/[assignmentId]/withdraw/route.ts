...
import { withdrawTransportationAssignment } from "@/lib/supabase/transportation";
...
export async function POST(request: Request, context: { params: Promise<{ assignmentId: string }> }) {
  ...
  const result = await withdrawTransportationAssignment({ assignmentId, actorUserId: auth.user.id, reason });
