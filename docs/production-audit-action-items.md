...
10. Prove brand profiles across the 20 launch surfaces.
    ...
    - Action: run `QA_PROOF_BASE_URL=<hosted-url> npm run qa:brand-proof` and `npm run qa:pwa-cache-proof`, then create several hosted test brands with distinct logo URLs, banner URLs, primary/secondary/accent/button colors, display names, short names, fallback avatars, and hero copy before browser-testing parent team switching, invite pages, email templates, push identity, and stale-brand avoidance after a deployed brand revision bump.
