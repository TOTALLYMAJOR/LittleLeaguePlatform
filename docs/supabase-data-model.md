...
The active LeaguePilot Supabase production project is installed and migration-history readback is aligned at 39 migrations through `20260726144407_restore_anon_rls_policy_evaluation.sql`. On 2026-07-26, a clean PostgreSQL 17 reset and the empty-data Supabase preview `gmrvnnkxksqkcxcmydhr` first applied `0022` through `0033` plus six timestamped hardening migrations; after explicit approval, the same reviewed 18-migration gap was applied to production without seed data. A later clean reset and preview-only promotion applied migration 40, `20260726182645_optimize_rls_auth_initplans.sql`. That additive migration follows [Supabase's row-invariant RLS guidance](https://supabase.com/docs/guides/database/postgres/row-level-security#call-functions-with-select) by wrapping 72 `auth.uid()` calls across 49 policies in scalar subqueries; it changes no policy name, command, role, permissiveness, normalized predicate, grant, or RLS setting. Production has not applied migration 40. The first local pass exposed `authorization` as a reserved alias in `0028` and `0029`; after those aliases were replaced, the chain installed. Transportation now permits multiple pending offers while a partial unique index enforces only one final assignment. Error-level lint also found and the additive chain repaired an ambiguous `revocation_reason` reference in the additional-guardian revocation function.
...
The hosted preview proves ordered application through `20260726182645`, migration readback, a no-op follow-up plan, unchanged Data API grants and RLS enablement, real-session parent/coach/anonymous boundaries, and the provider-free `0028`/`0029` lifecycle. Its Performance Advisor moved from 224 warnings to 175 by clearing all 49 `auth_rls_initplan` findings; the 175 `multiple_permissive_policies` findings remain open. Production migration-history and schema readback remains through `20260726144407`, with unchanged preflight-sampled row counts, complete RSVP backfill, empty new workflow tables, disabled provider flags, and 224 warning-level RLS performance findings. A guarded production dry run lists only `20260726182645`; it was not applied. Neither target's evidence proves production signed-in browser/session behavior, every feature lifecycle, Realtime change delivery, provider behavior, backup/PITR/restore, or operational acceptance. See `docs/supabase-migration-rehearsal-2026-07-26.md`.
...
The shared ordered chain is installed through `20260726144407` on isolated preview `gmrvnnkxksqkcxcmydhr` and in production. Preview alone has applied and read back the additive RLS initplan optimization at `20260726182645`; production remains at 39 migrations and its guarded dry run lists that one candidate as pending. `npm run qa:rls-proof` and `npm run qa:migration-gap-proof` ran only on preview after migration 40, covering real-session parent/coach/anonymous policies plus the provider-free transportation and caregiver happy/denial/revocation path. Cross-organization, cross-team, cross-family, competing-offer, wrong-role, expiry, cache-clearing, correction, downstream-refusal, production signed-in browser/session behavior, Realtime, provider behavior, and backup/PITR/restore remain pending after schema promotion and before operational acceptance.
