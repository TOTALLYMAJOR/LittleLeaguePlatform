...
The verifier reads repository files only. It checks that program-summary money truth, legacy compatibility rows, placement, fulfillment, and public display remain separate; proof-only status and browser return messages do not claim Stripe settlement; one-time sponsor collection uses server-side Checkout Sessions when enabled; Stripe keys stay server-side; missing Stripe configuration fails closed; and signature-verified webhooks remain the only settlement truth. It also checks the atomic sponsor webhook boundary, provider-resource replay guard, refund/dispute handling, service-role-only execution, restricted-key guidance, separate environments, and absence of committed Stripe secret values.
...
`supabase:qa-users` creates or updates fictional QA users on the guarded target. `qa:rls-proof` signs in through the anon key and verifies role boundaries. `qa:session-proof` is isolated-QA-only: it performs the shared registration-counter burst/readback, registration review and assigned-team activation, team-builder publish/replay, parent actions, link-media Hide/Restore, media-consent grant/revoke, calendar/weather authorization denials, provider-draft checks, screenshots, and service-role readback. `qa:tenant-readiness-proof`, `qa:demo-tenant-proof`, `qa:brand-proof`, and `qa:coordination-proof` likewise prove only the selected target and commit.
...
Before hosted public and tenant readiness browser proof, run the no-mutation hosted readiness preflight with the intended isolated QA/Preview URL, target public organization configuration, public access review-window copy, all three QA role credentials, service-role readback credential, and invocation-only migration target:

```bash
QA_PROOF_BASE_URL=https://<isolated-preview-host> PUBLIC_ORGANIZATION_ID=<organization-uuid> PUBLIC_ACCESS_REVIEW_WINDOW='within two business days' NEXT_PUBLIC_SUPABASE_URL=https://<project-ref>.supabase.co NEXT_PUBLIC_SUPABASE_ANON_KEY=<anon-key> SUPABASE_SERVICE_ROLE_KEY=<service-role-key> SUPABASE_POOLER_DATABASE_URL=<pooler-url> SUPABASE_MIGRATION_TARGET_REF=<project-ref> SUPABASE_MIGRATION_TARGET_ENV=preview SUPABASE_MIGRATION_ALLOW_APP_TARGET=confirmed-nonproduction-target QA_PARENT_EMAIL=<qa-parent@example.com> QA_PARENT_PASSWORD=<qa-parent-password> QA_COACH_EMAIL=<qa-coach@example.com> QA_COACH_PASSWORD=<qa-coach-password> QA_ADMIN_EMAIL=<qa-admin@example.com> QA_ADMIN_PASSWORD=<qa-admin-password> npm run qa:hosted-readiness-preflight
```

The preflight only validates inputs and prints the migration dry-run plus public-family, signed-session, RLS, and tenant-readiness commands. It does not deploy, bypass Vercel Authentication, seed Supabase, write hosted data, send providers, write payments, upload media, apply migrations, or establish production acceptance. Passing it clears obvious blockers before browser proof; it is not hosted acceptance.
...
The verifier reads repository source only. It checks that media report, media moderation, team-builder publish, broader admin export scope, and public intake abuse-control contracts remain present in route handlers, Supabase adapters, migrations, and focused tests. Public intake must use the service-only shared counter, return `429` with bounded headers after the configured burst, and return retryable `503` without writing when the shared store is unavailable; process-memory fallback is forbidden.

Passing this command is not hosted acceptance. It does not call Supabase, sign in, run Playwright, seed data, mutate hosted records, send providers, deploy, or configure edge/firewall controls. `npm run qa:session-proof` closes the corresponding isolated-target evidence only when it observes five allowed invalid registration attempts, a sixth-request `429`, the required headers, and a six-hit shared Supabase bucket delta at the exact guarded hosted target.
