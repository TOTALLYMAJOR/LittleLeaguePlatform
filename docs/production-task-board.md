...
| Phase 1 - Access and activation | Request receipt/timeline, privacy-minimized status/recovery, atomic first-sign-in preferences, and one-time identity-matched invitation acceptance implemented locally; remainder in progress | Apply and prove migrations `0025` and `0026`, implement approved invitation issuance/delivery, and complete additional guardian invitation. Existing request status remains `pending`, `approved`, or `rejected`; first-sign-in performs no provider send. Acceptance activates only the child/team scope already present in the invitation and guardian record. |
