...
- Status: The complete chain installs on PostgreSQL 17. Isolated preview `gmrvnnkxksqkcxcmydhr` is at 40 migrations through `20260726182645_optimize_rls_auth_initplans.sql`; its plan is empty, all 49 initplan warnings are cleared, and the remaining 175 performance warnings are overlapping permissive policies. After explicit approval, production `dkwghvvlbdnnwzbnscvu` applied the earlier reviewed 18-migration gap without seed data and remains at 39 migrations through `20260726144407_restore_anon_rls_policy_evaluation.sql`. Production readback confirms RLS on all 92 public tables, the intended 58-table/20-table Data API split, `btree_gist` in `extensions`, the valid field-reservation exclusion constraint, unchanged application row counts, and zero error-level lint/advisor findings. A production dry run lists migration 40 only; it was not applied. Preview real-session parent/coach/anonymous RLS plus provider-free `0028` transportation and `0029` caregiver lifecycle proof pass after migration 40; those populated journeys were not run against production.
...
- Supabase promotion snapshot (verified 2026-07-26): preview `gmrvnnkxksqkcxcmydhr` applied/read back `20260726182645_optimize_rls_auth_initplans.sql` as migration 40, its guarded follow-up plan is empty, and its Performance Advisor moved from 224 warnings to 175 by clearing all 49 `auth_rls_initplan` findings. Preview normalized policy, grant, and RLS-state digests are unchanged, and parent/coach/anonymous RLS plus populated provider-free transportation/caregiver proof pass after the migration. Production `dkwghvvlbdnnwzbnscvu` was not mutated by this follow-on and remains at 39 migrations through `20260726144407_restore_anon_rls_policy_evaluation.sql`; its guarded dry run lists migration 40 only. Production error-level [database lint](https://supabase.com/docs/guides/database/database-linter) and advisors report no findings, Security Advisor warnings remain zero, and the last verified Performance Advisor state remains 224 warnings: 49 initplan plus 175 overlapping permissive policies. All 92 public tables have RLS. Preflight-sampled application row counts are unchanged, all five RSVPs were backfilled to the current schedule version, all 34 new workflow tables remain empty, and provider sends remain disabled. Production remains open for migration-40 approval/readback, signed-in browser/session and broader lifecycle proof, Realtime behavior, the remaining permissive-policy review, provider behavior, and backup/PITR/restore acceptance; PITR is disabled and no restore drill is proven. See `docs/supabase-migration-rehearsal-2026-07-26.md`.
