...
| Phase 1 - Access and activation | Request receipt/timeline, privacy-minimized status lookup, enumeration-safe invitation recovery, and atomic first-sign-in preferences implemented locally; remainder in progress | Apply and prove migration `0025`, then complete signed single-use invitation acceptance and additional guardian invitation. Existing request status remains `pending`, `approved`, or `rejected`; first-sign-in saves the adult’s language/privacy/channel choices but performs no provider send and changes no guardian or official operational truth. |
