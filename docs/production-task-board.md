...
| Phase 1 - Access and activation | Request receipt/timeline, privacy-minimized status lookup, and enumeration-safe invitation recovery implemented locally; remainder in progress | Complete signed single-use invitation acceptance, first-sign-in language/notification setup, and additional guardian invitation. Existing request status remains `pending`, `approved`, or `rejected`; no new workflow state or access mutation was introduced by the public lookup/recovery slice. |
