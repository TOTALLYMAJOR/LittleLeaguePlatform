...
- Status: The complete candidate now installs on PostgreSQL 17 and on the isolated Supabase preview branch `gmrvnnkxksqkcxcmydhr`. The preview is aligned through `0033` plus six timestamped hardening migrations, including Data API grant compatibility, `btree_gist` extension relocation, the additional-guardian revocation repair, and explicit RLS-helper grants. Error-level database lint is clean; real-session parent/coach/anonymous RLS plus provider-free `0028` transportation and `0029` caregiver lifecycle proof pass. The active LeaguePilot production project remains unchanged at `0021`.
...
- Proof queue: retain the isolated preview while completing cross-organization, cross-team, cross-family, wrong-role, concurrency, expiry, cache-clearing, correction, and downstream-refusal coverage. Repeat signed-in hosted role/browser proof, review production backup/PITR/restore posture, and obtain explicit production approval before any `main` migration. Provider, media, and payment slices remain separate allowlisted sandbox and hosted gates.
...
- Supabase promotion snapshot (verified 2026-07-26): the active LeaguePilot production project remains through `0021` and was not mutated. After the plan upgrade, an empty-data preview branch was created at `gmrvnnkxksqkcxcmydhr`. A guarded dry run and session-pooler apply installed `0022` through `0033` plus six timestamped hardening migrations, ending at `20260726144407_restore_anon_rls_policy_evaluation.sql`; history aligns and the next dry run is empty. Local and hosted error-level [database lint](https://supabase.com/docs/guides/database/database-linter) reports no findings. All public tables have RLS. The 58 legacy tables retain RLS-governed Data API DML, while the 20 `0022`-`0024` server tables explicitly revoke browser roles and grant service-role DML only. The broad default `PUBLIC` execution grant is removed from eight RLS helpers, while `anon` and `authenticated` retain the explicit execution required for policy evaluation. Hosted parent/coach/anonymous RLS and populated provider-free transportation/caregiver proof pass. Security Advisor warnings for those policy helpers, intentionally policy-less service tables, [leaked-password protection](https://supabase.com/docs/guides/auth/password-security#password-strength-and-leaked-password-protection), broader feature lifecycles, browser proof, backup/restore review, and production approval remain open. See `docs/supabase-migration-rehearsal-2026-07-26.md`.
...
| Phase 1 - Access and activation | Local implementation complete; migrations `0025`-`0027` and `0033` are installed on isolated preview | Run the populated invitation/additional-guardian lifecycle, signed-in cross-family RLS, and responsive browser proof. Provider delivery remains an independent external gate. Existing request status remains `pending`, `approved`, or `rejected`; no slice silently sends a provider message or expands approved child/team scope. |
...
| Phase 3 - Responsibility and temporary care | `0028`/`0029` install locally and on isolated preview. Hosted proof passes for request → offer → mutual acceptance and caregiver create → wrong-email rejection → exact-email acceptance → revoke, including audit, schedule-version, token-rotation, no-membership, and zero-notification checks. | Add same-team competing-offer, cross-team/cross-family, expiry, cache-clear, and signed-in browser proof. Production remains unchanged. |
| Phase 4 - Priority communication and disruption | Local immutable versions, correction/withdrawal, exact schedule-version binding, four-surface projection, visible propagation incidents, current-version acknowledgment, and responsive proof complete; `0030` is installed on isolated preview | Run populated one-revision projection, incident lifecycle, offline/accessibility, provider sandbox/webhook, and hosted browser proof. |
| Phase 5 - Parent Replay and season continuity | Local published-only family story, private engagement, consent-aware optional media, memory timeline, reviewed transition/source archival/safe correction, explainable readiness, and responsive proof complete; `0031`/`0032` are installed on isolated preview | Run populated media consent/revocation, retention, multi-guardian transition concurrency, expiration/downstream refusal, accessibility, moderated-family, and hosted browser proof. |
