...
- Validation: `lib/supabase/tenant-readiness.test.ts`; `components/feature-panels.test.tsx`; `app/route-guards.test.ts`; `app/routes-smoke.test.ts`; `npm run qa:tenant-readiness-proof`; `npm run typecheck`; `git diff --check`.
