...
| Phase 3 - Responsibility and temporary care | Local outbound/return transportation request, driver offer, requester acceptance, restriction fail-closed behavior, withdrawal correction, Event Passport projection, and responsive degraded-state proof complete | Apply/prove `0028` in isolated QA, then implement time-bound caregiver identity, minimum scope, dual acceptance, expiry, revocation, and cache clearing. Local empty-database SQL installation is not currently available because this workspace has neither `psql` nor WSL Docker integration; the production project remains unchanged. |
