...
The shell-convergence portion of this map is implemented locally. Route topology now declares `surfaceFamily`, `shellFamily`, `primaryNavigationFamily`, and optional `familyMobileTab` metadata. The shell resolver combines that metadata with a server-derived confirmed active context; a broad `canParent` capability by itself cannot select the family shell. On ambiguous shared routes for a confirmed multi-role adult, the resolver preserves the last explicit role context and otherwise falls back to neutral chrome.
...
- Parent-owned routes plus `/account`, `/team-chat`, and `/team-portal` retain the family shell only when the confirmed or preserved active context is Parent. Staff-owned routes retain the staff shell. `/access/status` and `/invite/accept` are explicit neutral transition surfaces. Caregiver and public authority boundaries remain separate.
- The family shell owns the warm light theme even when the device requests dark mode, removes the staff sidebar/video and duplicate context bars, and integrates role, organization, season, team, linked-player count, and access status into the compact header.
...
Authenticated local proof is recorded by `scripts/capture-family-shell-proof.mjs` and `output/playwright/family-shell/proof.json`. It covers seven family-context routes and one staff regression route at 320, 390, 768, 1024, and 1440 pixels. Axe critical/serious checks apply to every family-context result; Coach remains a shell-regression check because its pre-existing dark-surface contrast is outside LP-UX-001.
