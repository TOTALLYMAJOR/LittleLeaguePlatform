...
- **Theme selection (amended 2026-08-03):** every route starts in light mode unless the user explicitly selects Dark. The saved `light` or `dark` choice applies across public, Family, staff, account, and shared routes; device `prefers-color-scheme` no longer changes the app automatically. Dark mode was admitted only after the authenticated 13-route Family matrix passed at 390px and 1440px with zero serious/critical axe findings, zero light content-panel leaks, zero document overflow, and zero browser errors (`lp-ux-005-manual-global-theme.md`).
...
The existing `qa:contrast-proof` remains the broad token-pair harness. `qa:manual-theme-proof` now authenticates demo parent, coach, and admin roles; covers all Family routes in selected dark mode at mobile and desktop widths; verifies light-default and persisted selection behavior; rejects light content-panel leaks; and runs axe AA checks. Remaining known defects outside that exercised matrix (`secondary danger`, gateway sub-ink, disabled-button paint) stay regression candidates. Contrast claims in docs cite proof artifacts, not intent (repo rule).
