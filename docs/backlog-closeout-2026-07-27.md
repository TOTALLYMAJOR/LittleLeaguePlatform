...
| Order | Work item | State | MVP completion condition |
| --- | --- | --- | --- |
| 1 | LPM-020 hosted public-configuration contract | `done-local`; hosted replay in order 8 | Expose only the irreversible organization fingerprint and configured review-window evidence; fail closed when either is missing or mismatched. No raw organization UUID, credential, secret, cookie, token, or client-side authority. Local contract tests, typecheck, build, and the 16-result browser matrix pass. |
| 2 | LP-005 registration approval and assigned-team activation | `done-local`; hosted readback in order 8 | Registration review reads are organization-scoped; approval/acceptance derives the assigned team from persisted registration/invite rows; newly issued `#token` links open correctly; focused authority, route, adapter, and invitation tests pass. Hosted player/guardian/invitation/action/audit readback remains order 8. |
| 3 | LP-007 team-builder publication | `done-local`; isolated-QA publish in order 8 | Admin-only Preview -> Edit -> Approve -> Publish persistence, stale-plan refusal, organization scope, and audit contracts pass 17 focused tests plus the admin readiness verifier. The guarded hosted-session harness now performs preview, approval, atomic publish, identical-action replay, assignment/action/audit readback, and screenshot capture; executing it against the intended isolated target remains order 8. |
| 4 | LP-009 admin tenant scope | `done-local`; hosted denial proof in order 8 | Registration, branding, media, memberships, guardian repair, archives, operations, schedule, and export reads require explicit organization/team scope. Cross-tenant adapter and ICS route tests pass. Hosted populated denial remains order 8. |
| 5 | Security defect fixes | `done-local` | ICS export requires active requested-team membership/admin access; media consent has a session-attributed parent UI plus atomic guardian-authorized/audited RPC; weather drafts require assigned-coach/org-admin access before provider lookup. Chat read-receipt authorization and the chat-retention no-op remain postponed. |
| 6 | LP-010 minimum public-intake abuse control | `done-local`; deployed-store proof in order 8 | Registration and mobile intake use the durable Supabase `claim_public_rate_limit` counter with bounded headers and `429` behavior. Shared-store failure now returns retryable `503` with no intake write, and process-memory fallback is forbidden. Fourteen focused tests plus the admin readiness verifier pass; the guarded session harness requires a five-allowed/sixth-`429` burst and a six-hit shared-counter delta. Executing that proof remains order 8. |
| 7 | LP-003 / LP-004 link-media moderation contraction | `done-local`; hosted toggle proof in order 8 | The staff workbench exposes one state-dependent Hide/Restore control, the route accepts only `hidden`/`approved`, authorization/audit history remains intact, and family/team reads exclude hidden rows. Upload, storage, scanner, parent-report, rejection, destructive removal, and separate approval controls are postponed. |
| 8 | EXT-HOSTED-SESSION | `external` | Definition of shipped: exact deployed commit/environment, ordered migrations applied and read back, target identity proven, and signed-in parent/coach/admin core journeys passing against the intended isolated hosted tenant. Preflight now requires the complete role/readback/migration-plan inputs. The guarded session harness covers shared-counter burst/readback, registration player/guardian assigned-team activation, team-builder publication/replay, link-media Hide/Restore, media consent, calendar/weather denials, provider-draft boundaries, and core role routes. Local work is ready; publication is intentionally paused to avoid CI credits. |
...
| --- | --- | --- | --- |
| EXT-HOSTED-SESSION | `external` | Release owner with an isolated QA deployment and QA Supabase authority | This is the current definition of shipped. Install and read back the complete ordered migration chain on an explicitly identified isolated QA project, deploy the intended commit to a separately identified QA/Preview app, pass the fail-closed preflight, then run its ordered public-family, guarded signed-session, RLS, and tenant-readiness commands without targeting a production alias. Preserve the exact deployment SHA, migration plan/apply/readback, role screenshots, team-builder action/assignment/audit readback, consent audit readback, denial results, and shared-counter evidence. |
