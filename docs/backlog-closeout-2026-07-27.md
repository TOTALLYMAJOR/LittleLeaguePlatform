...
| 6 | LP-010 minimum public-intake abuse control | `done-local`; deployed-store proof in order 8 | Registration and mobile intake use the durable Supabase `claim_public_rate_limit` counter with bounded headers and 429 behavior; six focused tests and the admin readiness verifier pass. Deployed shared-store execution remains order 8. |
...
| 8 | EXT-HOSTED-SESSION | `external` | Definition of shipped: exact deployed commit/environment, ordered migrations applied and read back, target identity proven, and signed-in parent/coach/admin core journeys passing against the intended isolated hosted tenant. Preflight now requires the complete role/readback/migration-plan inputs and the guarded session harness covers registration, team-builder publication, media consent, calendar/weather denials, and core role routes. Local work is ready; publication is intentionally paused to avoid CI credits. |
