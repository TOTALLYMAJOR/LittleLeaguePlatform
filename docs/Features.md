...
Feature slices live in the root Next.js application with typed fallback states and Supabase-backed paths for auth-scoped reads, writes, audits, provider-safe drafts, and admin operations. The [2026-07-27 closeout ledger](backlog-closeout-2026-07-27.md) is authoritative for `done-local`, `external`, `decision-required`, and `historical` state. Real SendGrid, Twilio, Web Push, private-media, Stripe Connect, and OpenAI adapters remain behind server and organization gates; code presence is not hosted, operational, or production proof.

## Local Queue Closeout - 2026-07-27
...
| Automatic Team Builder | Phase 1 - Launch Readiness | Done local; isolated-QA browser/RLS acceptance pending | `/admin`; `/api/admin/team-builder-inputs`; `/api/admin/team-builder-plans` | `components/team-builder-workbench.test.tsx`; `app/api-team-builder.test.ts`; `lib/domain/domain.test.ts`; `lib/supabase/team-builder-inputs.test.ts`; `lib/supabase/team-builder-plans.test.ts`; `supabase/team-builder-production.test.ts`; migration `20260727145702_complete_private_team_builder_publish.sql` | Authorized admins can use private birthdate-derived age/age band, evaluation, sibling/guardian, friend-request, skill-balance, lock, and roster constraints through persisted Preview -> Edit -> Approve -> Publish plans with audit evidence. Parent/public child detail is not exposed. Connected isolated-QA publish/readback and cross-org browser proof remain `EXT-HOSTED-SESSION` and `EXT-RLS-ACTOR-ACTION`. |
| PWA, Mobile, Dark Mode | Platform Foundation | Done local; hosted reconnect proof pending | all routes, `/offline`, `/api/mobile-usage-events`, `/api/registration-requests` | `app/routes-smoke.test.ts`; `app/public-intake-rate-limit.test.ts`; `lib/offline/game-day-outbox.test.ts`; `components/offline-sync-status.test.tsx`; isolated-local browser proof; `npm test`; `npm run build` | Manifest, install prompt UX, offline fallback, responsive layout, dark mode, and actor/context-bound offline outbox foundations are present. The service worker uses network-first navigation with `/offline` fallback rather than cache-first private HTML. Environment, organization, current-session, role, context, version, and reconnect checks fail closed. PWA-first remains the deferred default; hosted reconnect/conflict proof and shared hosted rate limiting remain external. |
