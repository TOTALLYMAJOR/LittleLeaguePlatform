...
- LP-UX-001 converges confirmed Parent contexts on one topology-derived Family shell and exact Home / Schedule / Messages / Family / More navigation. This is local shell and navigation proof only; no route, domain, permission, provider, schema, staff behavior, deployment, or hosted acceptance changed.
...
| LP-UX-001 Family Shell Convergence | Product UX Foundation | Done-local; hosted route proof pending | `/parent/*`; `/parent/more`; Parent-context `/account`, `/team-chat`, `/team-portal`; neutral `/access/status`, `/invite/accept` | `lib/navigation/route-topology.test.ts`; `components/ui/AppShell.test.tsx`; `app/routes-smoke.test.ts`; `scripts/capture-family-shell-proof.mjs`; `output/playwright/family-shell/proof.json`; `npm run qa:family-shell-proof`; `npm run typecheck`; `npm test`; `npm run build` | Route topology explicitly classifies surface, shell, primary-navigation family, and Family mobile-tab placement. Only a server-confirmed or explicitly preserved Parent context selects the Family shell; a broad `canParent` capability is insufficient, ambiguous multi-role shared routes fail neutral, and staff-owned routes retain the staff shell. Mobile and desktop share exactly Home / Schedule / Messages / Family / More, secondary destinations live under `/parent/more`, Account owns sign out, and verified context is integrated into the header. The light Family surface is explicit even under dark device preference. Local authenticated proof covers seven family-context routes at 320/390/768/1024/1440 with zero critical/serious axe findings, plus a five-width Coach shell regression. No route moves, domain behavior, permission, provider operation, schema, deployment, or hosted acceptance was added. |
