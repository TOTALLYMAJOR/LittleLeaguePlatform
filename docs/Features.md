...
| Parent Dashboard | Phase 2 - Parent Engagement | Done | `/parent` | `lib/domain/domain.test.ts`; `components/feature-panels.test.tsx`; `npm test`; `npm run build` | Parent Home now uses the team-home layout: sticky team header, parent navigation, next-event card, RSVP row, weather draft/change rows, conditional action/message/photo summaries, and privacy footer. It stays scoped to linked child/team data and uses plain family setup, schedule alert, media, and help-request language. |
