...
- The Family Home 10/10 progression starts with a local presentation slice: `/parent` no longer embeds the Detailed family operations disclosure, the Home readiness rail now renders only "Nothing unresolved for Saturday" or an exact linked action list, and route-level loading/error states explain safe failure without implying any RSVP, ride, message, or access mutation. This is UI composition work over existing guardian-scoped data only; no domain, API, migration, provider, hosted, or production boundary changed.
- LP-UX-009 clears 14 non-domain lint warnings by removing dead feature-panel derivations, stabilizing offline replay effect inputs, using explicit local-preview Next images, correcting sortable-control ARIA, and deleting an unused proof helper. Full tests, typecheck, lint, and production build pass; the two remaining warnings are isolated to protected domain files and are explicitly deferred rather than changed implicitly.
- LP-UX-008 hardens the coach RSVP reminder-draft runtime boundary with strict scalar identifiers, session-derived actor authority, privacy-safe cross-scope failures, explicit HTTP conflict/unavailable semantics, observable audit partial failures, duplicate-reuse audit evidence, and direct adapter/degraded-read tests. It also clears the React theme-toggle lint error and browser-found shared navigation target/contrast defects; the final local production-build theme matrix passed all 32 scenarios. It retains the existing pending notification state and performs no provider send. Cross-instance uniqueness remains separately deferred schema work.
- LP-UX-007 completes the ordered action-clarity remediation: real coach RSVP reminder drafts, one enumerated coach task queue, focused coach work surfaces, true done states, inline action preconditions, a queue-only Admin home, shared shell-attention counts, a compact 390px context, canonical staff navigation, and actor-named queue copy. Local signed-in browser evidence is under `output/playwright/lp-ux-007-action-clarity/`; no hosted or provider acceptance is inferred.
...
| LP-UX-003 Schedule and RSVP Convergence | Product UX Foundation | Done-local-accepted; hosted proof pending | `/parent/schedule`; `/parent/rsvp`; `/api/rsvps` | `components/feature-panels.test.tsx`; `components/family/components.test.tsx`; `components/family/family-acceptance-contract.test.tsx`; `components/ui/AppShell.test.tsx`; `app/routes-smoke.test.ts`; `scripts/capture-schedule-rsvp-proof.mjs`; `output/playwright/lp-ux-003-schedule-rsvp/proof.json`; `output/playwright/lp-ux-003-contrast/proof.json`; focused tests; `npm run qa:schedule-rsvp-proof`; focused `npm run qa:contrast-proof`; `npm run typecheck`; `npm run build`; `git diff --check` | Schedule owns guardian-scoped inline RSVP actions, honors the shared family filter, and opens the existing Event Passport without inventing arrival time. The compatibility RSVP route is a Needs Reply task list. Both use the same three-answer, version-bound, authenticated control and the existing API/RPC conflict path. Local responsive acceptance covers both routes at 320/390/768/1024/1440 with no document overflow, undersized audited controls, serious/critical axe findings, page errors, or non-aborted request failures. Eight additional authenticated route/mode results pass the family-light, device-light, device-dark, and forced-colors contrast harness. The interaction proof intercepts success, `schedule_changed`, and `guardian_conflict` responses without mutating hosted rows or calling providers. Schedule date and Event Passport controls now meet the 44px family target floor, and the shared RSVP hover/focus paints preserve contrast after a conflict. The shared control refuses offline writes and does not claim a queued response. No schema, route contract, provider send, access grant, or attendance authority changed. |
| LP-UX-004 Truthful Family Utilities | Product UX Foundation | Done-local-accepted; hosted proof pending | `/parent/settings`; `/parent/more`; `/account`; `/parent/practice-recaps`; `/coach/practice-recaps` | `components/family-first-sign-in.test.tsx`; `components/feature-panels.test.tsx`; `components/ui/AppShell.test.tsx`; `lib/navigation/route-topology.test.ts`; `app/route-guards.test.ts`; `app/routes-smoke.test.ts`; `scripts/capture-family-utilities-proof.mjs`; `output/playwright/lp-ux-004-family-utilities/proof.json`; `output/playwright/lp-ux-004-contrast/proof.json`; focused tests; `npm run qa:family-utilities-proof`; focused `npm run qa:contrast-proof`; `npm run typecheck`; `npm run build`; `git diff --check` | Family Settings now renders the existing language, critical/routine channel, quiet-hours, translation, and shared-device preference writer instead of a duplicated dashboard. Media copy explicitly separates preferences from consent and family release. More exposes Practice Replays, Photos, Transportation, Settings, Account, Support, and offline status through guarded topology. Account uses plain-language access labels without rendering raw membership identifiers, preserves the active-role shell, names the current organization in staff branding when available, and keeps safe sign-out. Practice Replays is the converged product label while route paths remain compatible. Local responsive acceptance covers Settings, More, Account, and the parent Practice Replays route at 320/390/768/1024/1440 with no document overflow, undersized audited controls, text below 12px, serious/critical axe findings, page errors, or non-aborted request failures. Sixteen additional authenticated route/mode results pass family-light, device-light, device-dark, and forced-colors contrast proof. Settings save and Practice Replay save are intercepted locally, More keeps its exact seven approved destinations, and Account settles before its plain-language/privacy audit; hosted rows mutated and provider calls executed remain zero. The Practice Replay page and nested timeline now contain intrinsic grid width at 320px, and timeline status text uses the existing 12px token. No provider delivery, media consent, membership grant, schema, API, route, or product-surface expansion was added. |
| LP-UX-005 Family Photos and Portal Split | Product UX Foundation | Done-local; responsive and hosted proof pending | `/parent/photos`; `/team-portal`; `/api/media/report`; `lib/supabase/team-portal.ts` | `components/family-photos.test.tsx`; `components/family-team-page.test.tsx`; `lib/supabase/route-scopes.test.ts`; `app/route-guards.test.ts`; focused tests; `npm run typecheck`; `npm run build`; `git diff --check` | Family Photos receives a minimized guardian-scoped projection and displays only moderated media with explicit family-release evidence. Visible private items also retain scan and signed-URL requirements from the existing adapter. The screen names the consent boundary, has no consent writer, explains the empty release pipeline, and uses the existing authenticated report route. Parent-context `/team-portal` now returns a minimized Family Team page with next-event, coach-name, Schedule, Messages, and help-board links; it does not serialize the portal branding editor, acting-user selector, or capability inventory. Coach/admin portal rendering remains unchanged. No schema, media-consent mutation, storage policy, provider delivery, or staff authority changed. |
| LP-UX-006 Family Access and Transportation Progression | Product UX Foundation | Done-local; responsive and hosted proof pending | `/parent/family-access`; `/parent/transportation`; existing guardian, caregiver, transition, and transportation mutation routes | `components/family-access-progression.test.tsx`; `components/additional-guardian-access.test.tsx`; `components/temporary-caregiver-access.test.tsx`; `components/season-transition-review.test.tsx`; `components/family-transportation.test.tsx`; `lib/supabase/transportation.test.ts`; focused tests; `npm run typecheck`; `npm run build`; `git diff --check` | Family Access is one child-first progression: select the child/team, choose permanent league-verified Guardian or temporary event-boxed Caregiver, then identify, scope, review, confirm, and track in the existing workflow. Pending season transitions appear only when guardian action is actually waiting. Guardian and caregiver scopes remain legally and operationally distinct; temporary care remains capped at 14 days. Transportation now presents the existing request, offer, and mutual-confirmation contract as a three-step path, keeps outbound and return separate, and names status/history in family language without exposing home address or restriction detail. No lifecycle state, mutation contract, schema, RLS, provider send, or authority boundary changed. |
| LP-UX-007 Communication Room Density | Product UX Foundation | Done-local; responsive and hosted proof pending | `/parent/messages`; existing notification acknowledgment, team-chat read-receipt, and team-chat reply routes | `components/communication-room.test.tsx`; `components/family/family-acceptance-contract.test.tsx`; `scripts/capture-communication-room-proof.mjs`; focused tests; `npm run typecheck`; `npm run build`; `npm run qa:communication-room-proof`; `git diff --check` | Communication Room retains three distinct Critical, Updates, and Conversation authority lanes plus family/team filtering and search. Source freshness is one honest summary with disclosed component status; event metadata and authority boundaries move behind accessible disclosures; receipt stages remain separate from acknowledgment; and the existing acknowledgment copy still excludes attendance, agreement, transportation, completion, and permissions. At family viewport sizes, lane tabs stay visible and only the selected lane is presented as the current view; conversation event context moves into that view. Desktop retains the contextual rail and all three scan lanes. Reply permissions, read receipts, mutation paths, official revision truth, provider evidence, and child-account privacy remain unchanged. |
...
| App-wide Weekly Dashboard Visual System | Product UX Foundation | Local implementation and signed-in responsive browser proof complete; hosted proof pending | Shared shell and primitives across public, parent, coach, admin, registration, team portal, and communication routes | `app/globals.css`; `app/layout.tsx`; `app/routes-smoke.test.ts`; `output/playwright/lp-ux-004-landing-gateway/sidebar-theme-proof.json`; `npm run qa:season-certainty-proof`; `npm run typecheck`; `npm test`; `npm run build` | The warm cream, navy, white, and restrained-orange language established on Parent Home now drives the global semantic tokens, Geist/Fredoka type hierarchy, shared shell, navigation, context bars, cards, controls, notices, public header, and dark-mode equivalents. The signed-in local matrix covered parent, schedule, transportation, caregiver, coach, Parent Replay, admin, and admin schedule surfaces at 375, 390, 768, 1024, and 1440 pixels with no browser errors or document overflow. It also corrected 1024px overflow in the temporary-caregiver builder and admin schedule control room. The staff sidebar critical background, border, inactive navigation, and video scrim now follow the same light/dark tokens as the staff canvas; local signed-in `/coach` and `/account` proof confirms AA token-pair contrast in both device preferences while preserving the subdued video treatment. Existing route structure, server-derived role context, permissions, state labels, Supabase adapters, child-privacy defaults, provider gates, and delivery behavior are unchanged. The visual migration uses the existing global CSS system rather than adding a second component framework. |
...
| Parent Weekly Dashboard Visual Adaptation | Product UX Foundation | Local implementation and authenticated responsive browser proof complete; hosted route proof pending | `/parent`; `/api/rsvps`; `/api/parent/replays/[replayId]/engagement` | `components/parent-weekly-dashboard.test.tsx`; `scripts/capture-season-certainty-proof.mjs`; `output/playwright/parent-weekly-dashboard/proof.json`; `npm run typecheck`; focused tests; `npm run build`; signed-in 375/390/768/1024/1440 browser matrix | Parent Home now opens with a warm cream, navy, white, and restrained-orange weekly-dashboard composition adapted from the supplied Magic Patterns reference and continued by the Family Home 10/10 progression. Its player/week summary remains guardian-scoped; hero and schedule RSVP controls retain authenticated idempotency, lock-version, schedule-version, and guardian-conflict rules; home activities come only from published Parent Replays; coach updates come only from scoped announcements; and the readiness rail now renders only "Nothing unresolved for Saturday" or exact linked actions for RSVP, ride, schedule-change, and conflict evidence. The generated field visual contains no identifiable child. The native Detailed family operations disclosure was removed from Home so Event Passport, transportation, messages, family access, balance, support, and season records stay on canonical routes instead of nested inside `/parent`. The compact route header keeps schedule, messages, account, command navigation, verified role context, offline notices, and the mobile tab bar. Loading and error states now explain safe failure without implying a mutation. The retained local production-build proof rendered against a signed-in QA parent with exact document/client widths and no browser errors at every recorded viewport; new browser proof for this continuation remains pending. No provider send, access grant, child login, athlete ranking, mock streak, reaction count, or new persistence authority was added. |
...
| Coach Dashboard | Phase 2 - Coach Operations | Done | `/coach`, `/coach/schedule`, `/coach/messages`, `/coach/attendance`, `/coach/practice-recaps`, `/coach/roster`, `/coach/weather-fields`, `/coach/drafts`, `/coach/snacks-volunteers` | `lib/season-certainty.test.ts`; `lib/supabase/coach-rsvp-reminders.test.ts`; `lib/supabase/coach-drafts.test.ts`; `components/feature-panels.test.tsx`; `npm run qa:season-certainty-proof`; `npm test`; `npm run build` | Coach Home starts with one enumerated Game-Day Radar queue whose count equals its visible task rows. Per-family RSVP reminders save pending review-only email draft records; they do not send. The write path rejects malformed input, derives the actor from the session, verifies team/event/guardian scope, reuses existing pending drafts, records created/reused audit evidence, and exposes persistence-versus-audit partial failure. Drafts, claim-first snacks/volunteers, attendance reminders, and weather decisions have focused guarded routes. Schedule leads with the Resolution Room and keeps event editing behind a disclosure. Navigation uses the canonical names RSVPs, Parent Replay, and Team Portal. Role wrappers use active coach memberships before loading sensitive team data. |
| Admin Dashboard | Phase 1 - Launch Readiness | Done | `/admin`, `/admin/family-access`, `/admin/branding`, `/admin/security-audit`, `/admin/reports-archive`, `/admin/media-review`, `/admin/communications`, `/admin/schedule-venues`, `/admin/message-delivery-review`, `/admin/sponsors`, `/admin/operations`, `/admin/teams` | `lib/season-certainty.test.ts`; `components/feature-panels.test.tsx`; `app/routes-smoke.test.ts`; `npm run qa:season-certainty-proof`; `npm test`; `npm run typecheck`; `npm run qa:tenant-readiness-proof`; `QA_PROOF_BASE_URL=http://127.0.0.1:3020 npm run qa:brand-proof`; `QA_PROOF_BASE_URL=http://127.0.0.1:3020 npm run qa:session-proof`; `npm run build` | Admin Overview is now a linked health-and-review queue surface ending in the actionable queues; the legacy composer, sponsor editor, media review, planning, and lineup workbenches no longer render inline. A “Fix next hold” primary names the first resolving route, every queue names the league admin as next actor, and the same selector feeds page counts and navigation badges. Media, sponsors, schedule, and telemetry stay on guarded focused routes. Settings, Safety & Weather, Security, and Themes aliases redirect to Operations, Schedule & Venues, Security & Audit, and Branding. Message delivery review remains a record-review surface rather than a live-send claim. |
...
| PWA, Mobile, Manual Theme | Platform Foundation | Done local; hosted reconnect/theme proof pending | all routes, `/offline`, `/api/mobile-usage-events`, `/api/registration-requests` | `app/routes-smoke.test.ts`; `lib/theme.test.ts`; `app/public-intake-rate-limit.test.ts`; `lib/offline/game-day-outbox.test.ts`; `components/offline-sync-status.test.tsx`; `npm run qa:manual-theme-proof`; `npm run qa:native-app-decision-readiness`; `output/playwright/lp-ux-008-runtime-hardening/theme-regression/proof.json`; `npm test`; `npm run build` | Manifest, value-gated install prompt UX, offline fallback, responsive layout, usage metrics, durable public intake limiting, actor/context-bound offline outbox foundations, and an app-wide manual Light/Dark choice are present. First visit is light even on a dark-preference device; explicit selection is stored locally and applied before hydration across public, Family, staff, account, and shared routes. The current local production-build 32-result theme proof covers every Family route at 390px and 1440px plus public, coach, and admin states with no serious/critical axe findings, light-panel leaks, overflow, or browser errors; shared brand/breadcrumb targets and attention-badge contrast satisfy the audited states. The service worker uses network-first navigation with `/offline` fallback rather than cache-first private HTML. Environment, organization, current-session, role, context, version, and reconnect checks fail closed. PWA-first remains the deferred default; hosted theme persistence, hosted reconnect/conflict, production usage, push-provider, app-store, native-product, and production acceptance remain external. |
