...
| Automatic Team Builder | Phase 1 - Launch Readiness | Local implementation complete; hosted proof open | `/admin` | `components/feature-panels.test.tsx`; `lib/domain/domain.test.ts`; `supabase/rls-policy.test.ts`; `supabase/migrations/0034_team_builder_player_metadata.sql` | Roster maker now includes balanced team-builder previews with sibling/guardian grouping, friend-request consideration, explicit age-band and cutoff-age labels, player evaluation ratings/sources, skill-balance scores, target roster warnings, Preview -> Edit -> Approve -> Publish workflow, and admin-only migration tables for future persisted team-build plans. Preview output keeps player display to first name plus last initial and review-safe notes; full birthdates and private child detail are not required or displayed. Publishing updates player assignments only through an admin-reviewed domain action with audit proof. Hosted browser publish proof, Supabase migration apply/readback, cross-org proof, and production acceptance remain open. |
