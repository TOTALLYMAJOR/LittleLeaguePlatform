...
- **Production migration is installed and read back.** After explicit approval, the active LeaguePilot Supabase production project (`dkwghvvlbdnnwzbnscvu`) applied the reviewed 18-migration chain without seed data and is aligned at 39 migrations through `20260726144407_restore_anon_rls_policy_evaluation.sql`. This proves ordered schema installation and migration-history readback only.
...
- **Preview migration and focused lifecycle proof remain retained.** The no-production-data preview branch `leaguepilot-migration-gap-qa-20260726` (`gmrvnnkxksqkcxcmydhr`) applied and read back `20260726182645`, and its guarded follow-up plan is empty. Preview Performance Advisor warnings fell from 224 to 175 by removing all 49 `auth_rls_initplan` findings; the 175 `multiple_permissive_policies` findings remain. Parent, coach, and anonymous RLS checks plus the provider-free transportation and caregiver lifecycle proof pass after the migration. Production was not mutated by this follow-on and remains at 39 migrations through `20260726144407`.
- **Production schema boundaries were read back.** All 92 public tables have RLS. The 58 legacy tables retain RLS-governed Data API DML, the 20 server-adapter tables expose no browser-role DML and retain service-role DML, `btree_gist` is in `extensions`, and the field-reservation exclusion constraint remains valid. Preflight-sampled counts for organizations, profiles, players, guardians, RSVPs, field reservations, registration actions, notifications, and delivery attempts are unchanged; all five RSVPs have the current schedule version, provider sends remain disabled for both organizations, and error-level lint/advisors are clean.
- **Operational acceptance remains separate.** Production is control-plane healthy and public/Auth/Storage/Data API smoke checks pass, but signed-in production browser/session journeys, broader cross-tenant and feature lifecycles, provider/webhook behavior, Realtime change delivery, and warning-level RLS performance work remain open. Production still reports 224 performance warnings until the preview-proven initplan migration receives separate approval and readback; the remaining 175 overlapping permissive-policy findings require an actor/action review rather than mechanical consolidation. PITR is disabled, the latest platform backup predates promotion, and no restore drill has been proven. See `docs/supabase-migration-rehearsal-2026-07-26.md`.
