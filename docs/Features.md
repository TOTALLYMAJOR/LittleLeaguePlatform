...
| Multi-Theme System and Theme Designer | Platform Foundation | Done | `/team-portal`, `/admin/themes`, `/api/admin/theme-defaults` | `components/feature-panels.test.tsx`; `lib/domain/domain.test.ts`; `app/api-live-actions.test.ts`; `npm test`; `npm run build` | Theme presets exist for soccer, football, baseball, scouts, golf, tennis, swim, and generic. Assigned coaches/admins can apply presets, mascot, and colors through Supabase-backed writes, and admins can save tenant defaults for future teams. The admin console now includes a brand-profile launch validation model covering logos, banners, primary/secondary/accent colors, display names, fallbacks, web surfaces, invite/email surfaces, push identity, monitoring events, alerts, coach feedback, and acceptance criteria across 20 target features. Binary upload, email rendering, and push delivery remain provider-gated. |
...
| Admin Dashboard | Phase 1 - Launch Readiness | Done | `/admin` | `components/feature-panels.test.tsx`; `npm test`; `npm run build` | Admin view summarizes teams, registration queue, sponsors, notification architecture, sponsor billing proof, automatic team-builder previews, and readiness cards. |
