...
| Parent Replay | Signature Feature | Done | `/coach/parent-replay`, `/` | `components/feature-panels.test.tsx`; `lib/domain/domain.test.ts`; `npm test`; `npm run build` | Coach practice recap builder accepts 2-3 focus areas and generates 30-second, 2-minute, and 5-minute home activities plus coach-to-parent translations, aggregate micro-coaching streaks, a memory moment, coach video, parent tip, skill cards, team quest, and local notification records. No provider sends, AI provider, video hosting, or production persistence is active. |
| Team Portal Feature Hub | Tier 1-3 Product Surface | Done | `/team-portal`, `/` | `components/feature-panels.test.tsx`; `lib/domain/domain.test.ts`; `npm test`; `npm run build` | Team-scoped portal displays weekly digest, Game Day Calm Mode essentials, field maps, coach video library, parent education, coach-to-parent translation, skill cards, team quests, weather alert boundary, skill trees, season storybook, memory timeline, volunteer center, and AI learning-plan boundary. Assigned coaches and org admins can update portal colors and mascot in local state. |
| Branded Team Chat | Phase 2 - Parent Engagement | Done | `/team-chat` | `components/feature-panels.test.tsx`; `lib/domain/domain.test.ts`; `npm test`; `npm run build` | Team Chat uses each team's mascot and colors, adds a branded clubhouse header, quick-topic chips, pinned coach notes, Game-Day Questions, moderation controls, and local-only message posting. |
| Multi-Theme System and Theme Designer | Platform Foundation | Done | `/team-portal` | `components/feature-panels.test.tsx`; `lib/domain/domain.test.ts`; `npm test`; `npm run build` | Theme presets exist for soccer, football, baseball, scouts, golf, tennis, swim, and generic. Assigned coaches/admins can apply presets, mascot, and colors locally. |
...
| Registration System | Phase 1 - Access Readiness | Done | `/registration`, `/admin` | `lib/domain/domain.test.ts`; `components/feature-panels.test.tsx`; `npm test`; `npm run build` | Parent self-registration creates pending local review requests only; no auth account or child access is granted. |
| Snacks, Volunteers, Sponsors | Phase 2 - Community Operations | Done | `/team-portal`, `/coach`, `/admin` | `components/feature-panels.test.tsx`; `npm test`; `npm run build` | Local snack slots, volunteer signup records, and sponsor records are seeded and surfaced by team/coach/admin views. |
| PWA, Mobile, Dark Mode | Platform Foundation | Done | all routes | `npm run build`; browser verification | Manifest and service worker are present, responsive layout already stacks on mobile, and CSS supports system dark mode. |
