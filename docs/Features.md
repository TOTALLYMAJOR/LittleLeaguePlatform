...
| AI Coach Workspace | Signature Feature | Done | `/coach/parent-replay` | `components/feature-panels.test.tsx`; `lib/domain/domain.test.ts` | Deterministic review-only workspace creates New Parent Brief, Weekly Digest, Practice Replay, Announcement Cleaner, Smart FAQ, Coach Inbox Prioritization, Parent Brief Before Game, and Season Timeline drafts from existing announcements, schedule, pinned posts, team chat, media, volunteer needs, and coach-selected focus areas. Each draft shows source evidence and the Preview -> Edit -> Approve -> Publish workflow. No AI provider or automatic publish is connected. |
