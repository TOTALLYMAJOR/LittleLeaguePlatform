...
| Money + Sponsors Community Commerce | Phase 2 - Community Operations | Done | `/parent`, `/team-portal`, `/admin/sponsors`, `/api/parent/family-wallet`, `/api/admin/revenue-summary` | `lib/domain/domain.test.ts`; `components/feature-panels.test.tsx`; `app/api-live-actions.test.ts`; `npm run typecheck`; `npm run build` | Family Wallet summarizes registration fees, team dues, scholarship credits, sponsor discounts, unpaid balances, and proof states for the signed-in guardian's linked players. League Revenue Dashboard and Community Sponsor Matchmaker summarize admin-only receivables, sponsor invoice readiness, unpaid family balances, scholarship support, renewal risk, and suggested local sponsor opportunities. Local Business Team Pages render approved active team sponsor placements with offer/acknowledgment copy while hiding child profiles, parent contacts, private media, billing state, and redemption proof. Stripe collection, sponsor email sends, and payment settlement remain disconnected until webhook-confirmed proof is implemented. |
