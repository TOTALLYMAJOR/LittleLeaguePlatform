...
| Team Chat | Phase 2 - Parent Engagement | Done | `/team-chat`, `/api/team-chat/messages`, `/api/team-chat/moderation`, `/api/team-chat/read-receipts`, `/api/team-chat/reports`, `/api/team-chat/reports/review`, `/api/team-chat/retention/run` | `components/feature-panels.test.tsx`; `lib/domain/domain.test.ts`; `app/api-live-actions.test.ts`; `supabase/rls-policy.test.ts`; `npm test`; `npm run build` | Assigned parents/coaches/admins can use private team chat; no child accounts or direct messages. Messages, announcements, read receipts, moderation, report creation, report review, and retention cleanup execution use authenticated Supabase routes with session-derived actor IDs. Report rows stay manager-reviewed, moderation writes audit rows, and the retention runner invokes `purge_expired_team_chat_messages` plus an audit row. |
| Media Governance | Phase 2 - Parent Engagement | Done | `/parent`, `/team-portal`, `/admin`, `/api/media/report`, `/api/media/moderation`, `/api/media/uploads/intent`, `/api/media/uploads/finalize` | `lib/domain/domain.test.ts`; `lib/supabase/media-uploads.test.ts`; `app/api-live-actions.test.ts`; `components/feature-panels.test.tsx`; `npm test`; `npm run typecheck` | Parent/team reads load approved media only. Families can report media, coaches/admins can hide/restore/remove/set visibility, and authenticated team members/admins can create Supabase Storage upload intents and finalize uploaded JPEG/PNG/WebP/MP4 files. Upload object paths are tenant-scoped, rows remain `pending` with `scan_status=not_configured` until manual moderation, and retention/takedown policy metadata is stored. Hosted bucket proof and automated scanning provider proof remain pending. |
| Parent Replay | Signature Feature | Done | `/coach/parent-replay`, `/api/coach/parent-replay`, `/team-portal` | `components/feature-panels.test.tsx`; `lib/domain/domain.test.ts`; `app/api-live-actions.test.ts`; `QA_PROOF_BASE_URL=https://www.leaguepilot.us npm run qa:session-proof`; `npm test`; `npm run build` | Coach practice recap builder accepts 2-3 focus areas and generates home activities plus coach-to-parent translations, aggregate micro-coaching streaks, a memory moment, coach video, parent tip, skill cards, and team quest. Authenticated coach/admin publishing persists reviewed Supabase replay rows and pending parent notification drafts. Hosted QA browser proof covers Parent Replay publish rows and the pending provider-review boundary. No external provider send or AI provider runs. |
| AI Coach Workspace | Signature Feature | Done | `/coach/parent-replay`, `/api/coach/ai-workspace` | `components/feature-panels.test.tsx`; `lib/domain/domain.test.ts`; `lib/services/ai-coach/ai-coach-provider.test.ts`; `app/api/coach/ai-workspace/route.test.ts`; `QA_PROOF_BASE_URL=https://www.leaguepilot.us npm run qa:ai-coach-proof` | Deterministic review-only workspace creates New Parent Brief, Team Onboarding Brief for new coaches or added team participants, Weekly Digest, Practice Replay, Announcement Cleaner, Smart FAQ, Coach Inbox Prioritization, Parent Brief Before Game, Season Timeline, Coach Knowledge Base, Action Item Extraction, Safety Monitor, and End-of-Season Storybook drafts from existing announcements, schedule, pinned posts, visible team chat, roster names/jerseys, approved media, volunteer needs, and coach-selected focus areas. `/coach/parent-replay` now loads signed-in Supabase coach scope before provider requests, and missing coach access is gated. Signed-in assigned coaches/admins can request an OpenAI Responses API rewrite through the server route only when `AI_COACH_PROVIDER_ENABLED=true` and `OPENAI_API_KEY` are configured; requests use `store: false`, local privacy filters, source evidence, and the Preview -> Edit -> Approve -> Publish workflow. Evals cover cross-team data, private contacts, hidden media/messages, unsupported provider-send/publish claims, and unsourced private/external claims. Hosted QA proof captured `output/playwright/ai-coach-provider-rewrite-qa-session-live.png`. No automatic publish or provider send is connected. |
| Rookie Coach Assist | Coach Operations | Done | `/coach/parent-replay` | `components/feature-panels.test.tsx`; `lib/domain/domain.test.ts`; `npm test`; `npm run typecheck`; `npm run build` | Deterministic local generator helps new volunteer coaches plan age-safe motivation strategies and simple practice blocks for ages 3-6. Output includes Chaos Button 90-second sideline reset copy, Coach Voice Coach phrase replacement, Practice Personality Engine drill adaptation by team energy, Parent Replay seed focus areas, Parent Reinforcement Loop draft copy, source evidence, and safety boundary. No AI provider, automatic publish, external notification send, schema change, or workflow state is connected. |
| Team Portal Feature Hub | Tier 1-3 Product Surface | Done | `/team-portal`, `/` | `components/feature-panels.test.tsx`; `lib/domain/domain.test.ts`; `npm test`; `npm run build` | Team-scoped portal displays weekly digest, Game Day Calm Mode essentials, managed field maps, approved map embeds, venue markers, coach video library, parent education, coach-to-parent translation, skill cards, team quests, weather alert boundary, skill trees, season storybook, memory timeline, volunteer center, and AI learning-plan boundary. Assigned coaches and org admins can update portal colors and mascot through Supabase-backed APIs. |
| Branded Team Chat | Phase 2 - Parent Engagement | Done | `/team-chat` | `components/feature-panels.test.tsx`; `lib/domain/domain.test.ts`; `app/api-live-actions.test.ts`; `npm test`; `npm run build` | Team Chat uses each team's mascot and colors, adds a branded clubhouse header, quick-topic chips, pinned coach notes, Game-Day Questions, Supabase persistence, read receipts, Realtime subscription wiring, report buttons, manager report queue review, retention cleanup execution, and moderation controls. |
| Multi-Theme System and Theme Designer | Platform Foundation | Done | `/team-portal`, `/admin/themes`, `/api/admin/theme-defaults`, `/api/admin/team-logos` | `components/feature-panels.test.tsx`; `lib/domain/domain.test.ts`; `app/api-live-actions.test.ts`; `npm run qa:brand-proof`; `npm run qa:contrast-proof`; `npm test`; `npm run build` | Theme presets exist for soccer, football, baseball, scouts, golf, tennis, swim, and generic. Assigned coaches/admins can apply presets, mascot, and colors through Supabase-backed writes, and admins can save tenant defaults for future teams. `/admin/themes` is now an admin customization workbench for team identity, tenant defaults, logo metadata review, theme QA, launch validation, and audit context. Route-level light/dark/team contrast proof covers major parent, coach, admin, and team surfaces with failure screenshots. Admins can queue HTTPS logo asset metadata for review; binary upload, public rendering, email rendering, and push delivery remain provider-gated. |
| Coach Dashboard | Phase 2 - Coach Operations | Done | `/coach` | `components/feature-panels.test.tsx`; `npm test`; `npm run build` | Coach Home starts with next-event readiness, missing RSVP count, and the review list for weather, snack, and volunteer needs. Secondary cards use coach setup, team setup, and coach notes language. |
| Admin Dashboard | Phase 1 - Launch Readiness | Done | `/admin`, `/admin/operations`, `/admin/observability` | `components/feature-panels.test.tsx`; `app/routes-smoke.test.ts`; `lib/supabase/admin-observability.test.ts`; `npm test`; `npm run build` | Admin Home starts with league health: teams needing help, pending review queues, missing coaches/events, and family-link repairs. Operations covers provider inventory, approval queues, delivery triage, and audit logs. Observability aggregates auth failures, RLS denials, provider retries, webhook failures, public-intake throttles, admin actions, and media/chat moderation from Supabase-backed rows first; external dashboard hooks remain env-gated and optional. Message, billing, and team-builder sections stay review-only and provider-safe. |
| Registration System | Phase 1 - Access Readiness | Done | `/registration`, `/admin`, `/admin/registrations`, `/api/registration-requests` | `lib/domain/domain.test.ts`; `components/feature-panels.test.tsx`; `app/api-live-actions.test.ts`; `app/api-public-intake.test.ts`; `lib/supabase/public-rate-limit.test.ts`; `npm test`; `npm run build` | Parent self-registration creates pending review requests. Public registration intake is unauthenticated but durable-rate-limited with `Retry-After` and `X-RateLimit-*` headers. Admin approval/rejection flows create audited Supabase approval actions and derive reviewer identity from the verified session. |
| Snacks, Volunteers, Sponsors | Phase 2 - Community Operations | Done | `/team-portal`, `/coach`, `/admin`, `/api/snack-slots/claim`, `/api/snack-slots/unclaim`, `/api/snack-slots/reminders/draft`, `/api/volunteer-signups/claim`, `/api/volunteer-signups/unclaim`, `/api/volunteer-signups/reminders/draft`, `/api/admin/sponsors` | `components/feature-panels.test.tsx`; `app/api-live-actions.test.ts`; `lib/supabase/community-operations.test.ts`; `npm test`; `npm run build` | Snack and volunteer claims use authenticated Supabase APIs with cap checks, audit rows, controlled unclaim/reopen flows, and coach/admin reminder draft creation. Reminder drafts create pending `snack_reminder` or `volunteer_reminder` notification records only; provider sending remains approval-gated. Sponsor Management V2 supports admin-only CRUD, placement settings, logo asset rows, pending/active/expired status, audit events, and admin-only Stripe Product/Price/invoice/payment-proof readiness records. Live Stripe collection remains gated behind server-side restricted keys and provider configuration. |
...
| PWA, Mobile, Dark Mode | Platform Foundation | Done | all routes, `/offline`, `/api/mobile-usage-events` | `app/routes-smoke.test.ts`; `app/api-public-intake.test.ts`; `npm run qa:pwa-cache-proof`; `npm run qa:contrast-proof`; `npm test`; `npm run build` | Manifest, service worker, install prompt UX, offline fallback, responsive layout, system dark mode, route-level contrast proof, versioned service-worker caches, revisioned manifest/icon URLs, and PWA/native decision usage metrics are present. Anonymous usage telemetry is durable-rate-limited with shared-store fallback behavior. |
