...
| Provider Delivery Evidence | Provider Foundation | Adapter and webhook code; disabled by default | Internal notification worker; SendGrid/Twilio/Web Push adapters; verified webhook routes | Domain/provider unit tests; sandbox and hosted proof pending | Human approval, provider acceptance, verified delivery/failure, read, and explicit acknowledgment remain separate. Both environment and organization gates are required, QA recipients are allowlisted, webhook event IDs are replay-safe, and duplicate events are no-ops. No production SMS claim is allowed before consent, sender/A2P registration, opt-out, and cost-control proof. |
...
| Parent Dashboard | Phase 2 - Parent Engagement | Done | `/parent`, `/parent/schedule`, `/parent/messages`, `/parent/photos`, `/parent/practice-recaps`, `/parent/family-access`, `/parent/settings` | `lib/domain/domain.test.ts`; `lib/season-certainty.test.ts`; `components/feature-panels.test.tsx`; `npm run qa:season-certainty-proof`; `npm test`; `npm run build` | Parent Home starts with one mobile-first next-event surface using Today/This week grouping, a date rail, team context, time/location, directions, RSVP state, and honest entry points to the real RSVP workflow. `/parent/schedule` extends that pattern into Today, This week, and Later groups with stacked family-scoped event cards, confirmed/canceled/completed labels, private linked-player response state, directions, and a bright green RSVP entry button. The button opens `/parent/rsvp`; it does not save a response by itself. Weather, help needs, arrival timeline, pack checklist, field/parking plan, player readiness, and a copyable local game plan remain close by. Lower family tasks, media, logistics, and support screens are compressed behind disclosure rows. |
...
| Adaptive Calendar and Schedule Change Alerts | Phase 2 - Parent Engagement | Done | `/schedule`, `/parent/schedule`, `/coach/schedule`, `/admin/schedule-venues` | `components/feature-panels.test.tsx`; `lib/domain/domain.test.ts`; `npm run qa:season-certainty-proof`; `npm test`; `npm run build` | The calendar adapts by approved role without creating parallel data authority. Parent Schedule uses a Calm Agenda, seven-day ribbon, linked-player RSVP state, and an expandable read-only game-day command sheet. Coach Schedule adds a Now/Next/Later sideline timeline and an assigned-team readiness matrix derived from RSVP, snack, volunteer, and weather records. Admin Schedule uses a desktop split pane with month calendar, selected-event inspector, conflict/family/draft counts, and a Change Lens comparing original with proposed time, location, and status. Admin/coach edits still show impact preview and create review records only; no provider sends execute from these displays. |
...
| Parent Replay | Signature Feature | Closed-loop implementation; new hosted proof pending | `/coach/practice-recaps`, `/coach/parent-replay`, `/parent/practice-recaps`, `/api/coach/parent-replay`, `/approve`, `/publish`, `/team-portal` | Component/domain/API/AI tests; hosted proof must be refreshed after migration `0023` | Save Draft, Review, Approve, and Publish are separate operations. Publication creates notification drafts only and does not send. The AI Trust Panel shows included/excluded sources, freshness, model/time, human-review requirement, refusal/validation state, and stored source hashes. AI cannot publish, notify, grant authority, or invoke operational tools. |
