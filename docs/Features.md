...
| PWA, Mobile, Manual Theme | Platform Foundation | Done local; hosted reconnect/theme proof pending | all routes, `/offline`, `/api/mobile-usage-events`, `/api/registration-requests` | `app/routes-smoke.test.ts`; `lib/theme.test.ts`; `app/public-intake-rate-limit.test.ts`; `lib/offline/game-day-outbox.test.ts`; `components/offline-sync-status.test.tsx`; `npm run qa:manual-theme-proof`; `npm run qa:native-app-decision-readiness`; `output/playwright/lp-ux-005-manual-theme/proof.json`; `npm test`; `npm run build` | Manifest, value-gated install prompt UX, offline fallback, responsive layout, usage metrics, durable public intake rate limiting, actor/context-bound offline outbox foundations, and an app-wide manual Light/Dark choice are present. First visit is light even on a dark-preference device; explicit selection is stored locally and applied before hydration across public, Family, staff, account, and shared routes. The local 32-result theme proof covers every Family route at 390px and 1440px plus public, coach, and admin states with no serious/critical axe findings, light-panel leaks, overflow, or browser errors. The service worker uses network-first navigation with `/offline` fallback rather than cache-first private HTML. Environment, organization, current-session, role, context, version, and reconnect checks fail closed. PWA-first remains the deferred default; hosted theme persistence, hosted reconnect/conflict, production usage, push-provider, app-store, native-product, and production acceptance remain external. |
