...
- LP-UX-002 implements and locally accepts the Saturday Ready reference surface on `/parent`: FamilyFilter, privacy-safe What Changed diffs over `event_change_logs`, EventPassport, shared RSVP control, ReadinessStrip, and StatusChip. RSVP writes still use the existing `/api/rsvps` route and `save_parent_rsvp_with_versions` RPC path; the change watermark is device-local presentation only. No schema, migration, API contract, provider send, coach/admin route, transportation workflow, acknowledgement semantic, deployment, or production acceptance changed.
...
| LP-UX-002 Saturday Ready Reference Surface | Product UX Foundation | Done-local-accepted; hosted route proof pending | `/parent`; `/parent/schedule`; `/parent/rsvp`; `/parent/transportation`; `/parent/messages`; `/parent/family-access`; `lib/supabase/event-change-log-reads.ts` | `components/parent-weekly-dashboard.test.tsx`; `lib/supabase/event-change-log-reads.test.ts`; `app/api-live-actions.test.ts`; focused tests; `npm run typecheck`; `npm run lint`; `npm test`; `npm run build`; `git diff --check`; `scripts/capture-family-shell-proof.mjs`; `output/playwright/lp-ux-002-family-shell/proof.json`; `docs/product-experience/leaguepilot/lp-ux-002-acceptance-verification.md` | Parent Home now renders the approved first viewport order: Family header with presentation-only FamilyFilter, What Changed when safe event diffs exist, tokenized EventPassport, inline shared RsvpControl when reply/review is required, and ReadinessStrip. The event-change adapter derives active guardian child scope, team, organization, and season on the server, bounds results, orders deterministically, maps privacy-safe actor labels, allowlists family-safe fields, and never returns raw audit JSON. Device-local change watermarks are scoped by user, organization, season, and family/team context and are advanced only after successful query/render. RSVP uses the existing authenticated route/RPC path with idempotency, lock-version, schedule-version, `schedule_changed`, `guardian_conflict`, retry, and persisted-success copy. Local proof records 80 route-viewport results at 320/390/768/1024/1440, zero page errors, zero request failures, and zero Family axe critical/serious findings. The old embedded detailed operations disclosure is removed; canonical routes remain reachable. No schema, migration, API contract, provider send, coach/admin route, transportation workflow, acknowledgement semantic, hosted proof, or production acceptance changed. |
