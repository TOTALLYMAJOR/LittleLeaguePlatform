...
- **Production remains unchanged.** Read-only verification still places the active LeaguePilot Supabase production project at migration `0021`; no migration from this rehearsal was applied there.
- **The candidate installs cleanly.** A PostgreSQL 17 reset applied the complete history from `0001` through `0033` plus six timestamped hardening migrations. The rehearsal fixed reserved `authorization` aliases in `0028`/`0029`, corrected transportation indexing so multiple offers can await the requester while only one assignment can become final, and repaired an ambiguous additional-guardian revocation function.
- **Hosted migration proof now exists.** After the Supabase plan upgrade, the no-production-data preview branch `leaguepilot-migration-gap-qa-20260726` (`gmrvnnkxksqkcxcmydhr`) was created from the `0021` lineage. Guarded dry run and apply completed through `20260726144407`; migration-history readback is aligned and the follow-up dry run is empty.
- **Data API and RLS boundaries were verified.** The compatibility migration preserves DML grants for the 58 legacy RLS-governed tables and explicitly revokes `public`, `anon`, and `authenticated` from the 20 server-adapter tables before granting service-role DML. All public tables have RLS. `btree_gist` was moved to `extensions`; local and hosted error-level database lint is clean. The hosted Security Advisor still reports the intentionally policy-less service tables, required authenticated/anonymous RLS helper execution, and leaked-password protection as separate review/configuration items.
- **Hosted real-session and lifecycle proof passes.** Parent, coach, and anonymous RLS checks pass. The populated provider-free proof covers transportation request → offer → mutual acceptance and caregiver create → wrong-email rejection → exact-email acceptance → revoke, with schedule-version, audit, token-rotation, no-membership, and zero-notification readback.
- **Production promotion is still separate.** Backup/restore review, broader cross-tenant and feature-lifecycle coverage, hosted browser proof, provider/webhook proof, and explicit production approval remain open. See `docs/supabase-migration-rehearsal-2026-07-26.md`.

| Feature | Phase | Status | Implemented routes | Verification | Notes |
| --- | --- | --- | --- | --- | --- |
| Season Launch Wizard and traced roster commit | Phase 1 - Launch Readiness | Local closed-loop implementation; migration installed on isolated preview; feature-specific hosted proof pending | `/admin/imports`; `/api/admin/roster-imports/audit`; `/api/admin/season-launch/commit`; `/rollback` | `lib/domain/domain.test.ts`; `supabase/rls-policy.test.ts`; migration `0024`; transactional PostgreSQL migration/workflow smoke; `npm test`; `npm run typecheck`; `npm run build` | Parses and normalizes CSV rows, blocks errors, requires explicit warning confirmation, stages source evidence, and commits players/guardian links/invites only after active organization-admin approval. Every created row carries import provenance. Safe rollback removes only provenance-created rows and stops when imported players have downstream RSVP, attendance, safety, health, learning, media-consent, caregiver-handoff, or family-balance activity. Commit and rollback are atomic RPCs and execute zero provider sends. Team, coach, schedule, and communications remain separate launch gates. |
