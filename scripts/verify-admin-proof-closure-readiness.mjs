...
export const DEFAULT_SOURCE_FILES = {
  ...
  publicIntakeLimiter: "lib/public-intake/rate-limit.ts",
...
function verifyPublicIntakeAbuseControls(sources, blockers) {
  requirePattern(
    ...
    ["publicIntakeLimiter", "registrationRoute", "mobileUsageRoute"],
    /windowMs:\s*60_000[\s\S]*requestTimestamps\s*=\s*new Map[\s\S]*export function checkPublicIntakeRateLimit[\s\S]*checkPublicIntakeRateLimit\(\s*"registration"[\s\S]*checkPublicIntakeRateLimit\(\s*"mobile_usage"/s,
    "Registration and mobile telemetry routes must use the shared fixed-window public-intake limiter."
  );
  requirePattern(
    blockers,
    sources,
  ...
  requirePattern(
    ...
    /status:\s*429,\s*headers:\s*publicIntakeRateLimitHeaders\(rateLimit\)[\s\S]*status:\s*429,\s*headers:\s*publicIntakeRateLimitHeaders\(rateLimit\)/s,
