...
const routeSpecs = [
  ...
  { role: "parent", name: "account", path: "/account", shell: "family", activeTab: "More" },
  { role: "parent", name: "team-chat", path: "/team-chat", shell: "family", activeTab: "Messages" },
  { role: "coach", name: "coach-home", path: "/coach", shell: "staff" }
];
...
async function verifyKeyboardFocus(page, spec, width) {
  ...
  assert.equal(focus.tagName, "A", `${spec.path} shell destination is not keyboard-focusable.`);
