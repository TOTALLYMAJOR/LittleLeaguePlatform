...
function verifyMobileUsageBoundaries(sources, blockers) {
  ...
  requirePattern(
    ...
    ["mobileUsageRoute", "publicIntakeRateLimitTest"],
    /checkPublicIntakeRateLimit\([\s\S]*"mobile_usage"[\s\S]*publicIntakeRateLimitHeaders[\s\S]*throttles anonymous mobile telemetry bursts per client IP/s,
    "Mobile usage intake must stay protected by public-intake rate limiting and burst tests.",
  );
  requirePattern(
    blockers,
    sources,
...
function verifyOfflinePwaShellReadiness(sources, blockers) {
  requirePattern(
    ...
    ["manifest", "layout"],
    /"start_url": "\/"[\s\S]*"scope": "\/"[\s\S]*"display": "standalone"[\s\S]*manifest:\s*"\/manifest\.webmanifest"/s,
  ...
  requirePattern(
    ...
    ["serviceWorker"],
    /const OFFLINE_URL = "\/offline"[\s\S]*"\/manifest\.webmanifest"[\s\S]*OFFLINE_URL[\s\S]*networkFirstNavigation[\s\S]*caches\.match\(OFFLINE_URL\)/s,
    "The service worker must explicitly wire /offline and the manifest while using bounded network-first navigation fallback.",
  );
  requireNoPattern(
    blockers,
    sources,
