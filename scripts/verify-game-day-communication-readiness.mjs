...
function verifyOfflineReconnectConflictBehavior(sources, blockers) {
  ...
  requirePattern(
    ...
    /const ENDPOINTS[\s\S]*rsvp:\s*"\/api\/rsvps"[\s\S]*attendance:\s*"\/api\/coach\/attendance"[\s\S]*coach_note:\s*"\/api\/coach\/event-notes"[\s\S]*hasOwnProperty\.call\(ENDPOINTS,\s*actionType\)[\s\S]*isOfflineActionAllowed\("rsvp"\)\)\.toBe\(true\)[\s\S]*isOfflineActionAllowed\("attendance"\)\)\.toBe\(true\)[\s\S]*isOfflineActionAllowed\("coach_note"\)\)\.toBe\(true\)[\s\S]*isOfflineActionAllowed\("publish"\)\)\.toBe\(false\)/s,
  ...
  requirePattern(
    ...
    ["offlineOutbox"],
    /result\.status === 409[\s\S]*kind:\s*"conflict"[\s\S]*state:\s*"conflict"[\s\S]*Server version changed[\s\S]*settleFailure\(claim,\s*classifyOfflineFailure\(response\),\s*now\)[\s\S]*break;/s,
  ...
  requirePattern(
    ...
    ["offlineOutboxTest"],
    /Waiting to sync[\s\S]*Retrying[\s\S]*Sync conflict[\s\S]*Sign-in required[\s\S]*Review required[\s\S]*syncedAt:\s*expect\.any\(String\)[\s\S]*not\.toHaveProperty\("payload"\)/s,
    "Focused tests must cover waiting, retry, conflict, sign-in, review, and payload-free synced receipt states."
  );
}
